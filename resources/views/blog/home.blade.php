    ...
    <div class="col-md-8">
        ...
        @else
            @foreach($posts as $post)
                <div class="article">
                    <h3> <a href="{{url('/post/view/'.$post->id.'/'.$post->slug)}}">{{$post->title}}</a></h3>
                    ...
                    @if (!is_null($post->img_url))
                        <img src="{{url('/uploads/'.$post->img_url)}}"  class="img-responsive">
                        <hr>
                    @endif
                        <p><?php echo strip_tags(substr($post->text,0,1000)).'....'; ?></p>
                        <a class="btn btn-primary" href="{{url('/post/view/'.$post->id.'/'.$post->slug)}}">Lihat
